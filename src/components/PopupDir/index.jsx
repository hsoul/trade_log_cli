...
const PopupDir = forwardRef(({ onSelect }, ref) => {
  ...
  >
    <div className={s.popupDir}>
      ...
      <div className={s.content}>
        ...
        <div className={s.title}>方向</div>
        <div className={s.expenseWrap}>
          {
            // expense.map((item, index) => <p key={index} onClick={() => choseType(item)} className={cx({[s.active]: active == item.id})} >{ item.name }</p>)
            dir.map((item, index) => <p key={index} onClick={() => choseType(item)} className={cx({[s.active]: active == item.id})} >{ item.name }</p>)
          }
        </div>
      </div>
    </div>
